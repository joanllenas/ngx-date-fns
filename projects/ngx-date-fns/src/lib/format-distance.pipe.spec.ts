...
describe('FormatDistancePipe Pure', () => {
  ...
  it('should display the distance between 2 July 2014 and 1 January 2015', () => {
    expect(
      createPipe().transform(new Date(2014, 6, 2), new Date(2015, 0, 1))
    ).toBe('6 months');
  });
  ...
  it('should display the distance from 1 January 2016 to 1 January 2015, with a suffix', () => {
    expect(
      createPipe().transform(new Date(2015, 0, 1), new Date(2016, 0, 1), {
        addSuffix: true
      })
    ).toBe('about 1 year ago');
  });
  ...
  it('should display the distance between 1 August 2016 and 1 January 2015 in Spanish', () => {
    expect(
      createPipe().transform(new Date(2016, 7, 1), new Date(2015, 0, 1), {
        locale: es
      })
    ).toBe('más de 1 año');
  });
  ...
  it('should display the distance between 1 August 2016 and 1 January 2015 in Spanish when locale provided via config', () => {
    ...
    conf.setLocale(es);
    const pipe = new FormatDistancePurePipe(conf);
    expect(pipe.transform(new Date(2016, 7, 1), new Date(2015, 0, 1))).toBe(
      'más de 1 año'
    );
  });
});
